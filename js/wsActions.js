...
export class WsActions {
  ...
  catchErr = async (func, arg, err) => {
    ...
    try {
      ...
      console.error("Unforseen error occured. Notifying user...\n" + error);
      try {
  ...
  async establish(uuid) {
    ...
    await this.catchErr(
      async (uuid) => {
        ...
        console.log(
          uuid
            ? "Got user with UUID: " + uuid
            : "New user. Sending " + user + " UUID"
        );
        if (uuid) {
        ...
        send(
          this.ws,
          buildPayload(user, isRegistered ? "registered" : "establish")
        );
        return user;
      },
      uuid,
      "Could not establish user"
    );
  }
  async register(data) {
    await this.catchErr(
      async (data) => {
        ...
        console.log(`User data. Username: ${name} , Email: ${email}`);
        //data validation here
        const payload = validateRegister(name, email);
